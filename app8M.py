...
def resolve_asset_path(*candidates):
    """Cari path asset dengan fallback case-insensitive untuk deploy di Linux/cloud."""
    for candidate in candidates:
        ...
        for root, _, files in os.walk(base_dir):
            for file in files:
                if file.lower() == normalized.lower():
...
def buat_sertifikat_modul1_pdf(nama_lengkap: str, instansi: str, tanggal_lulus: str):
    ...
    logo_path = resolve_asset_path("logo.png", "logo.PNG")
    if logo_path:
        logo_width = 80
        logo_height = 80
    ...
    ttd_path = resolve_asset_path("images/ttd.png", "Images/ttd.png", "ttd.png", "ttd.jpeg")
    try:
        ...
        if ttd_path:
            c.drawImage(ttd_path, width - 195, 166, width=95, height=38, mask='auto')
    except:
        pass
    ...
    try:
        ...
        logo_stamp_path = resolve_asset_path("logo.png", "logo.PNG")
        if logo_stamp_path:
            ...
            c.drawImage(logo_stamp_path, width - 131, 164, width=40, height=32, mask='auto')
...
# =========================================================================
# 🔐 FUNGSI VALIDASI DATA PROFIL
# =========================================================================
def validate_phone_number(phone_str):
    """Validasi nomor telepon Indonesia. Return (is_valid, cleaned_number, error_message)"""
    ...
    if not phone_str.isdigit():
        return False, "", "Nomor telepon hanya boleh berisi angka, tidak boleh ada simbol atau huruf."
    
    # Minimal 10 digit, maksimal 13 digit
    if len(phone_str) < 10 or len(phone_str) > 13:
        return False, "", "Nomor telepon harus 10-13 digit (contoh: 08123456789)."
    
    return True, phone_str, ""

def validate_store_name(store_str, allow_empty=True):
    """Validasi nama toko/instansi. Return (is_valid, cleaned_name, error_message)"""
    if not store_str or not store_str.strip():
        if allow_empty:
            return True, "", ""
        else:
            return False, "", "Nama toko/instansi tidak boleh kosong."
    
    store_str = store_str.strip()
    if len(store_str) < 2:
        return False, "", "Nama toko/instansi minimal 2 karakter."
    
    if len(store_str) > 100:
        return False, "", "Nama toko/instansi maksimal 100 karakter."
    
    return True, store_str, ""
...
if "profile_saved_notification" not in st.session_state: st.session_state.profile_saved_notification = False
...
# Tampilkan notifikasi profil tersimpan jika ada
if st.session_state.profile_saved_notification:
    st.sidebar.success("✅ Profil berhasil diperbarui!")
    st.session_state.profile_saved_notification = False
...
if st.sidebar.button("🏠 Kembali ke Beranda Utama", use_container_width=True, key="home_btn_top"):
    ...
    inisial = nama_lengkap[:1].upper() if nama_lengkap else "U"
    
    # Premium profile card dengan avatar bulat dan edit button
    profile_html = f"""
    <div style='
        background: linear-gradient(135deg, rgba(30,58,95,0.8) 0%, rgba(45,85,120,0.8) 100%);
        border-radius: 16px;
        padding: 16px;
        margin-bottom: 16px;
    ...
    '>
        ...
        <div style='
            ...
            line-height: 1.6;
        '>
            <div style='margin-bottom: 6px;'><strong>EMAIL</strong><br/>{st.session_state.user_email}</div>
            <div><strong>NO. HP</strong><br/>{handphone_user}</div>
        </div>
    </div>
    """
    st.sidebar.markdown(profile_html, unsafe_allow_html=True)
    
    # Edit Profile Expander - Premium Styling
    st.sidebar.markdown("""
    <style>
    .edit-profile-box {
        background: linear-gradient(135deg, rgba(30,58,95,0.5) 0%, rgba(45,85,120,0.5) 100%);
    ...
    [data-testid="stExpander"] {
        ...
        border: 1.5px solid rgba(255,152,0,0.25) !important;
        background: linear-gradient(135deg, rgba(15,30,50,0.4) 0%, rgba(25,45,70,0.4) 100%) !important;
    }
    </style>
    """, unsafe_allow_html=True)
    
    with st.sidebar.expander("✏️ Edit Profil", expanded=False):
        st.markdown("<div class='edit-profile-box'>", unsafe_allow_html=True)
        ...
        current_nama_depan = profile_data[0] if profile_data and profile_data[0] else ""
        current_nama_belakang = profile_data[1] if profile_data and profile_data[1] else ""
        current_jk = profile_data[2] if profile_data and profile_data[2] else "Laki-laki"
        current_instansi = profile_data[3] if profile_data and profile_data[3] else ""
        current_no_hp = profile_data[4] if profile_data and profile_data[4] else ""
        
        # Edit form dengan styling premium
        col_e1, col_e2 = st.columns(2, gap="small")
        with col_e1:
            new_nama_depan = st.text_input("👤 Nama Depan", value=current_nama_depan, key="edit_nama_depan_input", placeholder="Masukkan nama depan")
        with col_e2:
            new_nama_belakang = st.text_input("👤 Nama Belakang", value=current_nama_belakang, key="edit_nama_belakang_input", placeholder="Masukkan nama belakang")
        
        new_jk = st.selectbox("👥 Jenis Kelamin", ["Laki-laki", "Perempuan"], 
                              index=["Laki-laki", "Perempuan"].index(current_jk) if current_jk in ["Laki-laki", "Perempuan"] else 0,
                              key="edit_jk_input")
        
        new_instansi = st.text_input("🏢 Nama Toko / Instansi (Opsional)", value=current_instansi, key="edit_instansi_input", placeholder="Nama toko atau instansi Anda")
        
        new_no_hp = st.text_input("📞 No. Telepon", value=current_no_hp, key="edit_no_hp_input", placeholder="08123456789")
        
        st.text_input("📧 Email (Tidak Bisa Diubah)", value=st.session_state.user_email, disabled=True, key="email_readonly")
        
        st.markdown("</div>", unsafe_allow_html=True)
        
        col_btn1, col_btn2 = st.columns(2, gap="small")
        with col_btn1:
            save_clicked = st.button("💾 Simpan", use_container_width=True, key="save_profile_btn")
        with col_btn2:
            st.button("❌ Batal", use_container_width=True, key="cancel_profile_btn")
        
        if save_clicked:
            # Validasi nama depan
            if not new_nama_depan or len(new_nama_depan.strip()) < 2:
                st.error("❌ Nama depan minimal 2 karakter dan tidak boleh kosong.")
            else:
                # Validasi nomor telepon
                is_valid_hp, cleaned_hp, error_hp = validate_phone_number(new_no_hp)
                if not is_valid_hp:
                    st.error(f"❌ {error_hp}")
                else:
                    # Validasi nama toko (boleh kosong)
                    is_valid_instansi, cleaned_instansi, error_instansi = validate_store_name(new_instansi, allow_empty=True)
                    if not is_valid_instansi:
                        st.error(f"❌ {error_instansi}")
                    else:
                        # Simpan ke database
                        conn = hubungkan_db()
                        cursor = conn.cursor()
                        cursor.execute("""
                            ...
                            WHERE email = ?
                        """, (new_nama_depan.strip(), new_nama_belakang.strip(), new_jk, cleaned_instansi, cleaned_hp, st.session_state.user_email))
                        conn.commit()
                        conn.close()
                        
                        # Set notification flag dan reload
                        st.session_state.profile_saved_notification = True
                        st.rerun()
...
if st.session_state.login_sukses:
    ...
    [data-testid="baseButton-secondary"] {
        background: linear-gradient(135deg, rgba(255,152,0,0.2) 0%, rgba(251,140,0,0.2) 100%) !important;
    ...
    if st.sidebar.button("🚪 Keluar Akun (Log Out)", use_container_width=True, type="secondary", key="logout_btn_primary"):
...
# ----------------- KONDISI KETIKA USER SUDAH LOGIN SUKSES -----------------
    ...
    # --- JALUR ENGINE MULTI-MATERI BER-SENSOR SAKTI (MATERI 1 SAMPAI 20) ---
        ...
        if st.session_state.halaman_sekarang == 2:
            ...
            etle_1 = resolve_asset_path("images/etle-tilang-1.jpg", "Images/etle-tilang-1.jpg")
            etle_2 = resolve_asset_path("images/etle-tilang-2.jpg", "Images/etle-tilang-2.jpg")
            st.markdown("### 🖼️ Contoh Screenshot yang Menyerupai Modus Penipuan")
            col_img1, col_img2 = st.columns(2)
            with col_img1:
                if etle_1:
                    st.image(etle_1, caption="Screenshot 1 - Pesan tilang palsu", use_container_width=True)
            with col_img2:
                if etle_2:
                    st.image(etle_2, caption="Screenshot 2 - Lampiran file APK dalam modus ETLE", use_container_width=True)
