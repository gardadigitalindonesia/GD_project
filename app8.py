        ...
        st.markdown(
            ...
            <div style='display: flex; justify-content: center; align-items: center; text-align: center; margin: 0 auto;'>
                <div style='display: inline-block; text-align: center;'>
                    <img src='data:image/png;base64,{encoded}' alt='GardaDigital Logo' style='width: {width}px; max-width: 100%; height: auto; display: block; margin: 0 auto;' />
    ...
    st.components.v1.html(
        ...
        function resetScroll() {
            const targets = [
                ...
                window.parent && window.parent.document ? window.parent.document.body : null,
                window.parent && window.parent.document ? window.parent.document.documentElement : null
            ].filter(Boolean);
...
if "list_opsi_m17" not in st.session_state:
    st.session_state.list_opsi_m17 = [
        "Langsung buka attachment invoice dari email dan klik link untuk verifikasi pembayaran tanpa cek sender.",
        "Jangan buka attachment atau klik link dari email mencurigakan, langsung hubungi supplier/bank/marketplace melalui nomor resmi untuk konfirmasi, and laporkan email phishing ke tim keamanan.",
        "Teruskan email ke semua karyawan untuk meminta pendapat apakah email itu asli atau palsu.",
        "Abaikan email phishing karena kasir sudah berpengalaman and tahu mana email asli dan mana yang palsu."
    ]
    random.shuffle(st.session_state.list_opsi_m17)

if "list_opsi_m18" not in st.session_state:
    st.session_state.list_opsi_m18 = [
        "Langsung transfer uang ke rekening yang diberikan si penelepon karena kata-katanya terdengar urgent dan profesional.",
        "Menggugat klaim dan meminta bukti identitas (foto KTP), tidak langsung mengirim dana atau data sensitif, and cek nomor telepon melalui website resmi sebelum memberikan informasi.",
        "Memberikan nomor rekening toko and password e-banking kepada si penelepon agar bisa dicek sistemnya.",
        "Percaya begitu saja jika penelepon mengaku dari bank dan meminta update data pribadi via telepon."
    ]
    random.shuffle(st.session_state.list_opsi_m18)

if "list_opsi_m19" not in st.session_state:
    st.session_state.list_opsi_m19 = [
        "Langsung transfer uang kepada akun pembeli yang memiliki verifikasi blue check di Instagram/WhatsApp.",
        "Hindari berbagi informasi bisnis di media sosial publik, verifikasi akun seller sebelum berkomunikasi, and laporkan akun imposter ke tim support platform marketplace.",
        "Membagikan rekening dan nomor HP toko di bio Instagram publik agar pembeli bisa langsung kontak.",
        "Berasumsi semua akun dengan followers banyak and design cantik pasti asli and terpercaya."
    ]
    random.shuffle(st.session_state.list_opsi_m19)

if "list_opsi_m20" not in st.session_state:
    st.session_state.list_opsi_m20 = [
        "Menganggap social engineering adalah masalah yang jarang terjadi pada UMKM kecil.",
        "Lakukan edukasi rutin untuk tim UMKM tentang taktik social engineering, buat checklist verifikasi sender/caller, and bangun budaya untuk selalu skeptis terhadap permintaan data/uang urgent.",
        "Hanya training keamanan untuk pimpinan saja, tidak perlu untuk kasir atau admin operasional.",
        "Mengandalkan hanya pada password yang kuat tanpa perlu edukasi tentang cara mengenali manipulasi sosial."
    ]
...
# ----------------- KONDISI KETIKA USER SUDAH LOGIN SUKSES -----------------
    ...
    # --- HALAMAN 26: RAPOR AKHIR KOMPREHENSIF (SEMUA 20 MATERI) & SERTIFIKAT DIGITAL ---
        ...
        if skor_komulatif == 20:
            ...
            st.markdown(f"**Modul:** GardaDigital Modul 1 - Social Engineering & Pertahanan Siber")
            ...
            if st.button("🎖️ TERBITKAN SERTIFIKAT DIGITAL ANDA SEKARANG", type="primary", use_container_width=True):
                st.success("✅ Sertifikat Anda telah diterbitkan! Cek email untuk unduh PDF resmi.")
                st.info("📧 Sertifikat PDF akan dikirim ke email Anda dalam 24 jam.")
