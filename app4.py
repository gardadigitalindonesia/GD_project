...
# 1. Pengaturan Bawaan Layar Web
st.set_page_config(page_title="GardaDigital", page_icon="🛡️", layout="centered")

# =========================================================================
# 🧠 DATABASE INTERNAL: STRUKTUR DATA MATERI ASLI
# =========================================================================
DATA_SOAL_ASLI = {
    "m1": {
        "kunci": "Manusia memiliki celah emosi (panik/lengah) yang jauh lebih mudah dimanipulasi daripada menembus pertahanan kode keamanan digital bank.",
        "opsi": [
    ...
    "m2": {
        "kunci": "Mengabaikan SMS tersebut, lalu mengecek saldo secara mandiri lewat aplikasi Mobile Banking resmi atau datang ke mesin ATM terdekat.",
        "opsi": [
    ...
    "m3": {
        "kunci": "Memastikan lambang centang hijau resmi berada di sebelah kanan nama profil akun, bukan di dalam gambar foto profil bulatan.",
        "opsi": [
    ...
    "m4": {
        "kunci": "Mematikan telepon, langsung menghubungi nomor HP anak Anda secara mandiri, dan menyadari bahwa polisi asli tidak pernah meminta uang damai atau menyuruh warga berutang di Paylater.",
        "opsi": [
...
# =========================================================================
# 🤖 ENGINE INSTANT: FUNGSI PENGACAK JAWABAN YANG DIKUNCI DI MEMORI SERVER
# =========================================================================
def acak_seluruh_opsi_kuis():
    for mid, info in DATA_SOAL_ASLI.items():
        opsi_copy = list(info["opsi"])
        random.shuffle(opsi_copy) # Mengocok urutan kalimat secara acak 🎲
        st.session_state[f"list_opsi_{mid}"] = opsi_copy

# Inisialisasi database memori jangka pendek saat pertama kali aplikasi dibuka
...
# Jalankan pengocokan urutan jawaban jika memori opsi masih kosong melompong
if "list_opsi_m1" not in st.session_state:
    acak_seluruh_opsi_kuis()

KUNCI_MATERI = {mid: info["kunci"] for mid, info in DATA_SOAL_ASLI.items()}

# =========================================================================
# 🧭 SIDEBAR MINIMALIS BERSIH COMAN MODUL DROPDOWN
# =========================================================================
...
# =========================================================================
# 🔒 FILTER LOCKING SECURITY MODUL BERBAYAR
# =========================================================================
...
# =========================================================================
# 🏛️ EKSEKUSI JALUR HALAMAN LINEAR UTAMANYA MODUL 1
# =========================================================================
else:
    ...
    if st.session_state.halaman_sekarang == 0:
        ...
        st.write("Sistem ini menggunakan alur cerita linear bertingkat tanpa dropdown materi samping yang membingungkan. Setiap kali Anda masuk atau mengulang, **urutan pilihan kuis di bawah halaman akan diacak otomatis oleh sistem** demi akurasi evaluasi pertahanan siber Anda.")
        
        st.write("")
        if st.button("🚀 Mulai Pelatihan Sekarang (Materi 1)", type="primary", use_container_width=True):
        ...
        **Skenario Studi Kasus: Jebakan Toko Hijab Online**
        ...
        # Pengaman otomatis: Jika memori acakan Materi 1 belum dibuat, bikin detik ini juga
        if "list_opsi_m1" not in st.session_state:
            opsi_asli_m1 = [
                "Karena membobol akun perorangan bisa otomatis mendapatkan akses ke seluruh server pusat bank.",
                "Manusia memiliki celah emosi (panik/lengah) yang jauh lebih mudah dimanipulasi daripada menembus pertahanan kode keamanan digital bank.",
            ...
            import random
            random.shuffle(opsi_asli_m1)
            st.session_state.list_opsi_m1 = opsi_asli_m1
            
        opsi_acak = st.session_state.list_opsi_m1
        
        index_default = None
        if terkunci:
            jawaban_lama = st.session_state.jawaban_user.get("m1", "")
            if jawaban_lama in opsi_acak:
                index_default = opsi_acak.index(jawaban_lama)

        pilihan = st.radio("Pilih kalimat jawaban paling tepat di bawah ini:", opsi_acak, index=index_default, disabled=terkunci, key="radio_m1")
        ...
        if terkunci:
            ...
            if st.session_state.jawaban_user.get("m1") == "Manusia memiliki celah emosi (panik/lengah) yang jauh lebih mudah dimanipulasi daripada menembus pertahanan kode keamanan digital bank.":
                st.success("✅ **BENAR!** Penipu tahu meretas emosi manusia jauh lebih murah dan cepat daripada meretas kode server bank.")
            else:
    ...
    # ----------------- HALAMAN 2: MATERI 2 -----------------
        ...
        **Skenario Studi Kasus: SMS Tengah Malam Sang Pemilik Katering**
        ...
        # 📸 SIMULASI VISUAL SMS PALSU MATERI 2 (LINK RESMI UNTUK MENCEGAH IKON PATAH)
        st.image("https://unsplash.com", caption="Simulasi: Modus pengiriman link jebakan phising memanfaatkan kepanikan batas waktu finansial.", use_container_width=True)
        ...
        opsi_acak = st.session_state.list_opsi_m2
        index_default = None
        ...
        pilihan = st.radio("Pilih kalimat jawaban paling tepat di bawah ini:", opsi_acak, index=index_default, disabled=terkunci, key="radio_m2")
        ...
        if terkunci:
            st.warning("🔒 Jawaban Anda sudah dikunci dan tidak bisa diubah kembali.")
            if st.session_state.jawaban_user.get("m2") == KUNCI_MATERI["m2"]:
    ...
    # ----------------- HALAMAN 3: MATERI 3 -----------------
        ...
        **Skenario Studi Kasus: Layanan Pengaduan Gadungan**
        ...
        # 📸 SIMULASI VISUAL PROFIL CHAT MATERI 3 (LINK AKTIF ANTI-PATAH)
        st.image("https://unsplash.com", caption="Simulasi: Penipu memalsukan lencana lunas/terverifikasi resmi langsung di gambar profil bulatan.", use_container_width=True)
        ...
        opsi_acak = st.session_state.list_opsi_m3
        index_default = None
        ...
        pilihan = st.radio("Pilih kalimat jawaban paling tepat di bawah ini:", opsi_acak, index=index_default, disabled=terkunci, key="radio_m3")
        ...
        if terkunci:
            st.warning("🔒 Jawaban Anda sudah dikunci dan tidak bisa diubah kembali.")
            if st.session_state.jawaban_user.get("m3") == KUNCI_MATERI["m3"]:
    ...
    # ----------------- HALAMAN 4: MATERI 4 (STORI + DUKUNGAN MULTIMEDIA VIDEO BERITA RIIL) -----------------
    elif st.session_state.halaman_sekarang == 4:
        st.markdown("### 📘 MATERI 4: Vishing - Telepon Darurat Polisi Palsu & Jebakan Kuras Paylater")
        st.markdown("""
        **Kronologi Studi Kasus Nyata di Indonesia:**
        ...
        # 🎥 🚀 FITUR MULTIMEDIA VIDEO SAKTI: PUTAR VIDEO BERITA KORBAN PINJOL/PAYLATER DI INDONESIA
        st.subheader("📺 Saksikan Rekaman Video Kasus Nyata:")
        st.video("https://youtube.com")
        st.caption("Edukasi Siber: Rekaman video jurnalisme membongkar trik hipnotis telepon komplotan penipu online.")
        ...
        opsi_acak = st.session_state.list_opsi_m4
        index_default = None
        ...
        pilihan = st.radio("Pilih kalimat jawaban paling tepat di bawah ini:", opsi_acak, index=index_default, disabled=terkunci, key="radio_m4")
        ...
        if terkunci:
            st.warning("🔒 Jawaban Anda sudah dikunci dan tidak bisa diubah kembali.")
            if st.session_state.jawaban_user.get("m4") == KUNCI_MATERI["m4"]:
            ...
            if st.button("Buka Halaman Rapor Hasil Evaluasi Akhir 📊", use_container_width=True, type="primary"):
                ...
                st.rerun()
    # ----------------- HALAMAN 5: RAPOR EVALUASI AKHIR (DOUBLE PIE CHART MULTIMEDIA) -----------------
    elif st.session_state.halaman_sekarang == 5:
        st.markdown("### 📊 Rapor Hasil Evaluasi Saya")
        ...
        if skor_aman == 4:
            ...
            # TOMBOL ULANG YANG OTOMATIS MENGOMBINASIKAN FITUR PENGACAKAN ULANG (RESHUFFLE) 🎲
            if st.button("🔄 Ulangi Pelatihan Sekarang (Reset & Hitung Pengulangan)", type="primary", use_container_width=True):
                ...
                st.session_state.jawaban_user = {}
                st.session_state.materi_selesai = set()
                acak_seluruh_opsi_kuis()  # 🎲 Mengocok kembali seluruh pilihan jawaban ke formasi acak baru!
                st.session_state.halaman_sekarang = 1  # Balik ke Materi 1 otomatis
                st.rerun()
