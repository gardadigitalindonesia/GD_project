...
st.markdown(
    ...
    [data-testid="stSidebar"] .stTextInput input:-ms-input-placeholder {
    ...
    [data-testid="stSidebar"] .edit-profile-trigger {
    ...
    [data-testid="stSidebar"] button[kind="secondary"] {
        ...
        color: #fff !important;
        box-shadow: 0 10px 22px rgba(127, 29, 29, 0.34) !important;
    }
    ...
    @media (max-width: 768px) {
        ...
        [data-testid="stSidebar"] .premium-profile-card {
            padding: 0.8rem 0.7rem !important;
        }

        [data-testid="stSidebar"] .premium-profile-header {
            gap: 0.6rem !important;
        }

        [data-testid="stSidebar"] .premium-profile-avatar {
            width: 36px !important;
            height: 36px !important;
            font-size: 0.9rem !important;
        }

        [data-testid="stSidebar"] .premium-profile-name {
            font-size: 0.88rem !important;
        }

        [data-testid="stSidebar"] .premium-profile-role {
            font-size: 0.71rem !important;
        }

        [data-testid="stSidebar"] .premium-profile-item {
            padding: 0.38rem 0.5rem !important;
        }

        [data-testid="stSidebar"] .premium-profile-value {
...
def normalize_phone_number(raw_phone):
    phone = re.sub(r"[\s\-()]+", "", str(raw_phone or "")).strip()
    if not phone:
        return "", "Nomor HP wajib diisi."
    if not phone.isdigit() and not phone.startswith('+'):
        return "", "Nomor HP hanya boleh berisi angka atau awalan +."
    if phone.startswith('+'):
        phone = phone.replace('+', '')
    if not re.fullmatch(r"(?:62|0)8[1-9]\d{6,11}", phone):
        return "", "Format nomor HP tidak valid. Contoh: 0812xxxxxx atau +62812xxxxxx."
...
def normalize_store_name(raw_name, max_length=80, allow_blank=True):
    ...
    if len(name) > max_length:
        return "", f"Nama toko maksimal {max_length} karakter."
    if not any(ch.isalnum() for ch in name):
        return "", "Nama toko harus berisi huruf atau angka."
    if any(ch in "<>/\\" for ch in name):
        return "", "Nama toko mengandung karakter yang tidak valid."
...
def reset_scroll_ke_atas():
    # Streamlit sering menjaga posisi scroll lama di iframe. Untuk kasus materi, refresh browser satu kali
    # adalah cara paling stabil untuk memastikan user selalu melihat judul materi dari awal.
    if st.session_state.get("force_refresh_top", False):
        ...
        st.session_state.force_refresh_top = False
        st.stop()

    # Fallback ringan agar halaman tetap mulai dari atas saat kondisi normal.
    st.components.v1.html(
        """
        <script>
            try {
                ...
                console.log('fallback scroll reset:', e);
            }
        </script>
        """,
        height=0,
...
if os.path.exists("logo.png"):
    st.sidebar.markdown(
        """
        <div style='margin: -24px 0 0 0; padding: 0; text-align: center;'>
            <img src='data:image/png;base64,{}' style='width: 76px; max-width: 100%; height: auto; display: block; margin: 0 auto;' />
        </div>
        """.format(base64.b64encode(open("logo.png", "rb").read()).decode("utf-8")),
        unsafe_allow_html=True,
    )
else:
    st.sidebar.markdown("<h2 style='text-align: center; margin: -24px 0 0 0;'>🛡️</h2>", unsafe_allow_html=True)

st.sidebar.markdown("<div style='height: 0px;'></div>", unsafe_allow_html=True)
...
if st.session_state.login_sukses:
    ...
    st.sidebar.markdown(
        ...
        <div class="premium-profile-card">
            <div class="premium-profile-header">
                <div class="premium-profile-avatar">{escape(nama_lengkap[:1].upper() if nama_lengkap else 'U')}</div>
                <div>
                    <div class="premium-profile-name">{escape(nama_lengkap)}</div>
                    <div class="premium-profile-role">{escape(institusi_user)}</div>
                </div>
            </div>
            <div class="premium-profile-grid">
                <div class="premium-profile-item">
                    <span class="premium-profile-label">Email</span>
                    <div class="premium-profile-value">{escape(st.session_state.user_email)}</div>
                </div>
                <div class="premium-profile-item">
                    <span class="premium-profile-label">No. HP</span>
                    <div class="premium-profile-value">{escape(handphone_user)}</div>
                </div>
            </div>
        </div>
        """,
    ...
    with st.sidebar.expander("✏️ Edit Profil", expanded=False):
        st.markdown("<div style='height: 6px;'></div>", unsafe_allow_html=True)
        conn = hubungkan_db()
        cursor = conn.cursor()
        cursor.execute("SELECT nama_depan, nama_belakang, jenis_kelamin, instansi, no_hp FROM pengguna WHERE email = ?", (st.session_state.user_email,))
        data_profil = cursor.fetchone()
        conn.close()

        nama_depan_edit = data_profil[0] if data_profil and data_profil[0] else ""
        nama_belakang_edit = data_profil[1] if data_profil and data_profil[1] else ""
        jenis_kelamin_edit = data_profil[2] if data_profil and data_profil[2] else "Laki-laki"
        instansi_edit = data_profil[3] if data_profil and data_profil[3] else ""
        no_hp_edit = data_profil[4] if data_profil and data_profil[4] else ""

        col_edit_1, col_edit_2 = st.columns(2)
        with col_edit_1:
            nama_depan_baru = st.text_input("Nama depan", value=nama_depan_edit, key="edit_nama_depan")
        with col_edit_2:
            nama_belakang_baru = st.text_input("Nama belakang", value=nama_belakang_edit, key="edit_nama_belakang")

        jenis_kelamin_baru = st.selectbox("Jenis kelamin", ["Laki-laki", "Perempuan"], index=["Laki-laki", "Perempuan"].index(jenis_kelamin_edit) if jenis_kelamin_edit in ["Laki-laki", "Perempuan"] else 0, key="edit_jenis_kelamin")
        nama_toko_baru = st.text_input("Nama toko / instansi", value=instansi_edit, key="edit_nama_toko")
        no_hp_baru = st.text_input("No. HP", value=no_hp_edit, key="edit_no_hp")
        st.text_input("Email", value=st.session_state.user_email, disabled=True, key="edit_email_readonly")

        if st.button("💾 Simpan Perubahan Profil", use_container_width=True, key="simpan_profil_sidebar"):
            nama_depan_clean = nama_depan_baru.strip()
            nama_belakang_clean = nama_belakang_baru.strip()
            nama_toko_clean, error_nama_toko = normalize_store_name(nama_toko_baru, allow_blank=True)
            no_hp_clean, error_no_hp = normalize_phone_number(no_hp_baru)

            if not nama_depan_clean or len(nama_depan_clean) < 2:
                st.sidebar.error("Nama depan minimal 2 karakter dan tidak boleh kosong.")
            elif error_nama_toko:
                st.sidebar.error(f"⚠️ {error_nama_toko}")
            elif error_no_hp:
                st.sidebar.error(f"⚠️ {error_no_hp}")
            else:
                conn = hubungkan_db()
                cursor = conn.cursor()
                cursor.execute(
                    """
                    UPDATE pengguna
                    SET nama_depan = ?, nama_belakang = ?, jenis_kelamin = ?, instansi = ?, no_hp = ?
                    WHERE email = ?
                    """,
                    (nama_depan_clean, nama_belakang_clean, jenis_kelamin_baru, nama_toko_clean, no_hp_clean, st.session_state.user_email)
                )
                conn.commit()
                conn.close()
                st.sidebar.success("Profil berhasil diperbarui.")
                st.rerun()

    st.sidebar.markdown("<div style='margin: 0.75rem 0 0.5rem 0; padding: 0.45rem 0.55rem; border-radius: 10px; background: #f8fafc; color: #0f172a; font-size: 0.72rem; font-weight: 800; letter-spacing: 0.08em; text-transform: uppercase; box-shadow: inset 0 0 0 1px rgba(15, 23, 42, 0.08);'>Pilih Modul Pelatihan</div>", unsafe_allow_html=True)
    modul_terpilih = st.sidebar.selectbox(
        "",
        [
            "Modul 1: Social Engineering (Gratis)",
            "Modul 2: Digital Phishing (Gratis)",
            "Modul 3: Mengunci Brankas Bisnis (Gratis)",
            "Modul 4: Sertifikasi Akhir Modul (Gratis)"
        ],
        label_visibility="collapsed"
    )

    if modul_terpilih == "Modul 1: Social Engineering (Gratis)":
        st.session_state.halaman_sekarang = 1
        st.rerun()
    elif modul_terpilih == "Modul 2: Digital Phishing (Gratis)":
        st.session_state.halaman_sekarang = 1
        st.rerun()
    elif modul_terpilih == "Modul 3: Mengunci Brankas Bisnis (Gratis)":
        st.session_state.halaman_sekarang = 1
        st.rerun()
    elif modul_terpilih == "Modul 4: Sertifikasi Akhir Modul (Gratis)":
        st.session_state.halaman_sekarang = 1
        st.rerun()

    # semua modul dibuka tanpa batas premium agar semua materi aksesibel pas login

    if st.sidebar.button("🏠 Kembali ke Beranda Utama", use_container_width=True, key="sidebar_home"):
        st.session_state.intip_rapor_global = False
        st.session_state.halaman_sekarang = 0
        st.rerun()

    if st.sidebar.button("📊 Lihat Rapor Keseluruhan Saya", use_container_width=True, key="sidebar_rapor"):
        st.session_state.intip_rapor_global = True
        st.rerun()

    if st.sidebar.button("🚪 Keluar Akun (Log Out)", use_container_width=True, key="sidebar_logout", type="secondary"):
...
if not st.session_state.login_sukses:
    ...
    with tab2:
        ...
        if st.button("Daftar Akun Baru 📝", use_container_width=True):
            if email_reg == "" or pass_reg == "" or nama_depan == "" or nomor_hp == "":
                st.error("❌ Mohon lengkapi email, password, nama depan, dan nomor HP Anda!")
            elif "@" not in email_reg or "." not in email_reg.split("@")[-1]:
                st.error("❌ Format penulisan wajib menggunakan alamat email yang valid!")
            else:
                nama_toko_valid, error_nama_toko = normalize_store_name(nama_instansi, allow_blank=True)
                no_hp_valid, error_no_hp = normalize_phone_number(nomor_hp)

                if error_nama_toko:
                    st.error(f"❌ {error_nama_toko}")
                elif error_no_hp:
                    st.error(f"❌ {error_no_hp}")
                else:
                    conn = hubungkan_db()
                    cursor = conn.cursor()
                    try:
                        ...
                        cursor.execute("""
                            ...
                            VALUES (?, ?, ?, ?, ?, ?, ?)
                        """, (email_reg, pass_reg, nama_depan, nama_belakang, j_kelamin, nama_toko_valid, no_hp_valid))
                        conn.commit()
...
# ----------------- KONDISI KETIKA USER SUDAH LOGIN SUKSES -----------------
    ...
    # --- JALUR ENGINE MULTI-MATERI BER-SENSOR SAKTI (MATERI 1 SAMPAI 20) ---
        ...
        st.markdown(
            f"""
            <div style="display:flex; align-items:center; justify-content:space-between; gap:12px; margin-top: 0.5rem; margin-bottom: 0.35rem;">
                <h3 style="margin:0; font-weight:800; color:#0f172a;">📘 {judul}</h3>
                <a href="#puncak-garda-utama" title="Ke awal halaman" style="font-size: 1.2rem; color:#475569; text-decoration:none; opacity:0.85;">🔗</a>
            </div>
            """,
            unsafe_allow_html=True,
        )
        ...
        pilihan_user = st.radio("Pilih kalimat jawaban paling tepat di bawah ini:", opsi_tampilan, index=index_default, disabled=terkunci, key=f"radio_{mid}", label_visibility="collapsed")
        ...
        if terkunci:
            ...
            if st.button(teks_lanjut, use_container_width=True):
                ...
                st.session_state.force_refresh_top = True
