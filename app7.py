...
if "list_opsi_m13" not in st.session_state:
    st.session_state.list_opsi_m13 = [
        "Memberikan kode tersebut agar admin bisa membantu membatalkan pemindahan hadiah undian toko.",
        ...
        "Mengirimkan foto tangkapan layar SMS tersebut ke grup WhatsApp keluarga untuk meminta bantuan.",
        "Memasukkan kode tersebut ke dalam link marketplace untuk mengklaim voucher potongan ongkir."
    ]
    random.shuffle(st.session_state.list_opsi_m13)

if "list_opsi_m14" not in st.session_state:
    st.session_state.list_opsi_m14 = [
        "Langsung mengeklik file APK tersebut agar paket pesanan katering Anda tidak dibatalkan kurir.",
        ...
        "Meneruskan file tersebut ke pembeli untuk memastikan apakah barangnya sudah dikirim atau belum.",
        "Menginstal aplikasi tersebut di laptop kantor agar handphone operasional Anda tetap aman."
    ]
    random.shuffle(st.session_state.list_opsi_m14)

if "list_opsi_m15" not in st.session_state:
    st.session_state.list_opsi_m15 = [
        "Langsung mengirimkan pesanan katering karena pembeli sudah mengirimkan foto kertas struk bukti transfer resmi.",
        ...
        "Meminta pembeli mengirimkan foto KTP sebagai jaminan bahwa struk transfernya asli.",
        "Menghubungi layanan pelanggan bank untuk menanyakan mengapa uang transferan dari pembeli belum masuk."
    ]
    random.shuffle(st.session_state.list_opsi_m15)

if "list_opsi_m16" not in st.session_state:
    st.session_state.list_opsi_m16 = [
        "Mengubah password email akun marketplace Anda setiap minggu sekali.",
        ...
        "Menghapus seluruh riwayat obrolan chat WhatsApp toko agar tidak bisa dibaca oleh peretas.",
        "Membeli handphone baru khusus untuk operasional admin toko agar tidak bisa disadap."
    ]
...
# ----------------- KONDISI KETIKA USER SUDAH LOGIN SUKSES -----------------
    ...
    # --- JALUR ENGINE MULTI-MATERI BER-SENSOR SAKTI (MATERI 1 SAMPAI 6, 7, 8, 9, 10, 13-16) ---
    elif (2 <= st.session_state.halaman_sekarang <= 5) or (7 <= st.session_state.halaman_sekarang <= 10) or (12 <= st.session_state.halaman_sekarang <= 15) or (17 <= st.session_state.halaman_sekarang <= 20):
        ...
        # ----------------- HALAMAN 17: MATERI 16 (NEW!) -----------------
            ...
            next_hal = 21  # Gerbang jembatan menuju Halaman Rapor Akhir Batch 4 Pamungkas!
        else:
        ...
        if terkunci:
            ...
            teks_lanjut = f"Materi Selanjutnya (Materi {nomor + 1}) ➡️" if nomor not in [4, 8, 12, 16] else "Buka Halaman Rapor Evaluasi Akhir 📊"
    ...
    # ----------------- HALAMAN 18: RAPOR AKHIR BATCH 4 (MATERI 13 - 16 LULUS FINAL TOTAL!) -----------------
    elif st.session_state.halaman_sekarang == 21:
        st.markdown("### 📊 Rapor Akhir Evaluasi: Hasil Komulatif Pamungkas (Materi 1 - 16)")
        st.write("Selamat! Anda telah merampungkan seluruh materi pertahanan siber taktis dari GardaDigital. Berikut analisis total ketahanan siber komulatif Anda:")
        
        # 1. HITUNG SKOR KOMULATIF NYATA (TOTAL DARI 16 MATERI YANG SUDAH DIBUKA)
        skor_komulatif = 0
        for m_id in ["m1", "m2", "m3", "m4", "m5", "m6", "m7", "m8", "m9", "m10", "m11", "m12", "m13", "m14", "m15", "m16"]:
            if st.session_state.jawaban_user.get(m_id) == KUNCI_MATERI[m_id]:
                skor_komulatif += 1
        salah_komulatif = 16 - skor_komulatif

        # Tarik data status kelulusan and remidi dari database SQLite
        conn = sqlite3.connect("gardadigital.db")
        cursor = conn.cursor()
        try: cursor.execute("ALTER TABLE pengguna ADD COLUMN remidi_b4 INTEGER DEFAULT 0"); conn.commit()
        except sqlite3.OperationalError: pass
        try: cursor.execute("ALTER TABLE pengguna ADD COLUMN status_lulus_b4 INTEGER DEFAULT 0"); conn.commit()
        except sqlite3.OperationalError: pass
        
        cursor.execute("SELECT remidi_b1, remidi_b2, remidi_b3, remidi_b4 FROM pengguna WHERE email = ?", (st.session_state.user_email,))
        data_remidi = cursor.fetchone()
        conn.close()
        ...
        # 2. VISUALISASI SATU PIE CHART BESAR DI TENGAH
        col_tengah1, col_tengah2, col_tengah3 = st.columns([1, 2, 1])
        with col_tengah2:
            st.markdown("<p style='text-align: center; font-weight: bold; font-size:14px;'>🎯 Grafik Akurasi Komulatif Seluruh Kelas (Materi 1 - 16)</p>", unsafe_allow_html=True)
            fig_b4, ax_b4 = plt.subplots(figsize=(3, 3))
            ax_b4.pie([skor_komulatif, salah_komulatif], labels=[f"Aman ({skor_komulatif})", f"Celah ({salah_komulatif})"], colors=['#2ECC71', '#E74C3C'], autopct='%1.1f%%', startangle=90, textprops={'color':"white", 'fontsize':8})
            ax_b4.axis('equal')  
            fig_b4.patch.set_facecolor('#1E1E1E')
            ax_b4.set_facecolor('#1E1E1E')
            st.pyplot(fig_b4)

        st.markdown("---")
        
        # 3. LOGIKA KELULUSAN BATCH 4 PAMUNGKAS
        skor_b4_murni = sum([1 for m_id in ["m13", "m14", "m15", "m16"] if st.session_state.jawaban_user.get(m_id) == KUNCI_MATERI[m_id]])
        
        if skor_b4_murni == 4:
            st.balloons() # Balon perayaan kelulusan akhir dilepaskan kembali di sini! 🎈🎉
            st.success(f"👑 **CONGRATULATIONS! ANDA LULUS SEMUA GARDA UTAMA (16/16 BENAR)**\n\nLuar biasa, {st.session_state.user_email}! Anda resmi memiliki tingkat ketahanan siber akal sehat tertinggi. Akun bisnis, keuangan katering, and reputasi toko Anda kini terlindungi penuh oleh perisai digital professional!")
            
            # Kunci kelulusan final ke database
            conn = sqlite3.connect("gardadigital.db")
            cursor = conn.cursor()
            cursor.execute("UPDATE pengguna SET status_lulus_b4 = 1, halaman_terakhir = 19 WHERE email = ?", (st.session_state.user_email,))
            conn.commit()
            conn.close()
            
            if st.button("🏆 Terbitkan Sertifikat Kelulusan Resmi Digital Anda 🔓🚀", type="primary", use_container_width=True):
                st.session_state.halaman_sekarang = 19  # Menuju halaman lembaran cetak PDF otomatis besok!
                st.rerun()
        else:
            st.error(f"🚨 **STATUS EVALUASI BATCH 4: WAJIB REMIDI ({skor_b4_murni}/4 BENAR BATCH 4)**\n\nSistem mendeteksi operasional toko online Anda masih rentan lumpuh and dibajak penipu. Selesaikan remedial modul penutup ini!")
            
            if st.button("🔄 Ulangi Ujian Khusus Batch 4 (Kalimat di-Shuffle Kembali 🎲)", type="primary", use_container_width=True):
                conn = sqlite3.connect("gardadigital.db")
                cursor = conn.cursor()
                ...
                random.shuffle(st.session_state.list_opsi_m15)
                random.shuffle(st.session_state.list_opsi_m16)
                st.session_state.halaman_sekarang = 14  # Balik otomatis ke awal mula Batch 4 (Materi 13)
                st.rerun()
